...
export function checkMigration() {
  ...
  const currentVersion = game.settings.get('fvtt-yze-generic-stepped', 'systemMigrationVersion');
  const totalDocuments = game.actors.size + game.scenes.size + game.items.size;

  if (!currentVersion && totalDocuments === 0) {
    return game.settings.set('fvtt-yze-generic-stepped', 'systemMigrationVersion', game.system.version);
  }
...
export async function migrateWorld() {
  ...
  for (const a of game.actors.contents) {
    try {
      ...
      if (!foundry.utils.isEmpty(updateData)) {
        console.log(`yzegs | Migrating Actor entity ${a.name}`);
  ...
  for (const i of game.items.contents) {
    try {
      ...
      if (!foundry.utils.isEmpty(updateData)) {
        console.log(`yzegs | Migrating Item entity ${i.name}`);
  ...
  for (const s of game.scenes.contents) {
    try {
      ...
      if (!foundry.utils.isEmpty(updateData)) {
        console.log(`yzegs | Migrating Scene entity ${s.name}`);
...
export function migrateSceneData(sceneData) {
  const tokens = sceneData.tokens.map(token => {
    ...
    if (!t.actorId || t.actorLink) {
      ...
      const actorData = foundry.utils.deepClone(t.delta ?? {});
      actorData.type = token.actor?.type;
      if (!actorData.system) return t;
      ...
      foundry.utils.mergeObject(t.delta, update, { applyOperators: true });
    }
    return t;
  });
  return { tokens };
...
function _migrateWeaponProps(itemData, updateData) {
  ...
  updateData['system.props.sight'] = deleteField();
...
function _migrateWeaponReliability(itemData, updateData) {
  ...
  if (rel.score !== undefined) {
    ...
    updateData['system.reliability.score'] = deleteField();
  }
  return updateData;
}
...
function _migrateWeaponAmmo(itemData, updateData) {
  ...
  if (mag.target == undefined) {
    ...
    // Deletes old properties.
    updateData['system.mag.value'] = deleteField();
  }
  return updateData;
}
...
function _migrateVehicleReliability(actorData, updateData) {
  ...
  if (rel.score != undefined) {
    ...
    // Deletes old properties.
    updateData['system.reliability.score'] = deleteField();
    updateData['system.reliability.maxScore'] = deleteField();
  }
  return updateData;
}
...
function _migrateVehicleCrew(actorData, updateData) {
  ...
  if (oldCrew.driver != undefined) {
    ...
    // Deletes old properties.
    updateData['system.crew.driver'] = deleteField();
    updateData['system.crew.passenger'] = deleteField();
    updateData['system.crew.exposedPassenger'] = deleteField();
    updateData['system.crew.gunner'] = deleteField();
    updateData['system.crew.commander'] = deleteField();
  }
  return updateData;
}
...
function _migrateVehicleComponents(actorData, updateData) {
  if (actorData.system.components?.fuel?.value !== undefined) {
    ...
    updateData['system.components.ammo'] = deleteField();
    updateData['system.components.cargo'] = deleteField();
    updateData['system.components.externalStores'] = deleteField();
    updateData['system.components.weapon'] = deleteField();
    // Deletes old properties.
    updateData['system.tempComponents'] = deleteField();
  }
  return updateData;
}
...
function _migrateVehicleArmor(actorData, updateData) {
  ...
  if (armor.side?.value != undefined) {
    ...
    // Deletes old properties.
    updateData['system.armor.side'] = deleteField();
  }
  return updateData;
}
