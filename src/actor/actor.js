 ...
 */
export default class ActorYZEGS extends Actor {
  /* ------------------------------------------- */
  /*  Properties                                 */
  /* ------------------------------------------- */
  ...
  _prepareCharacterData(actorData) {
    ...
    // Gets the attribute values from their scores. Skill values are prepared by their Items.
   ...
   * @param {Object} obj system.attributes or any object with a "score" property
   * @private
   */
  _prepareScores(obj) {
    if ('score' in obj) {
  ...
  _computeVehicleEncumbrance(system, items) {
    ...
    const emptySeatWeight = emptySeatCount * YZEGS.vehicle.emptySeatEncumbrance;
    const extraPassengerCount = -Math.min(0, maxCrewQty - crewCount);
    const extraPassengerWeight = extraPassengerCount * YZEGS.vehicle.extraPassengerEncumbrance;
  ...
  addVehicleOccupant(crewId, position = 'PASSENGER', isExposed = false) {
    ...
    if (!YZEGS.vehicle.crewPositionFlags.includes(position)) {
      throw new TypeError(`yzegs | addVehicleOccupant | Wrong position flag: ${position}`);
    }
  ...
  async rollRadiationAttack(options) {
    ...
    return YZEGSRoller.taskCheck(rollConfig);
  }
  ...
  async applyDamageToCharacter(amount, attackData, sendMessage = true) {
    ...
    const template = 'systems/fvtt-yze-generic-stepped/templates/components/chat/character-damage-chat.hbs';
    const templateData = {
      name: this.name,
      initialAmount,
      amount,
